"""
Simulator-specific tests for thread-based SmoobuGateway methods.

These test behaviour that is intrinsic to the simulator (e.g. empty state,
internal ordering logic) and would not make sense as contract tests because
the real API always has existing data.

The adapter contract (applies to ALL implementations) lives in:
...
def test_get_threads_empty_when_no_messages():
    gw = SimulatorSmoobuGateway()
    gw.inject_active_reservation(_future_res(1))
    # No messages — reservation should not appear as a thread
